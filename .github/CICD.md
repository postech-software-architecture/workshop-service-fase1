...
| [`workflows/cd.yml`](workflows/cd.yml) | `workflow_dispatch` | Deploy no **EKS** (AWS Academy): autentica na AWS (credenciais temporárias com *session token*), gera o kubeconfig, cria o `workshop-secret`, injeta endpoint e nome do RDS e aplica `k8s/overlays/aws`, seta a tag imutável da imagem e aguarda o rollout. |
...
- **Variables** (não sensível): `AWS_REGION` (`us-east-1`), `EKS_CLUSTER_NAME` (`workshop-eks`) e
  `DB_NAME` (`workshop`, igual ao output `db_name` do Terraform).
